...
for DATASET in nq hotpotqa ms_marco; do
  CUDA_VISIBLE_DEVICES=4,5,6,7 PYTHONUNBUFFERED=1 python scripts/evaluate.py \
    --config configs/main/official_benchmark_500_learned_docpolicy.yaml \
    --checkpoint "${CHECKPOINT}" \
    --dataset "${DATASET}" \
    --n_questions "${N_QUESTIONS}" \
    --output "results/main/official_benchmark_500/learned_docpolicy/${DATASET}_${N_QUESTIONS}.md" \
